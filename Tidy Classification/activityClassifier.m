function [accuracy] = activityClassifier(trainingData)
...
for i=1:100
    ...
    Xtrain=trainingData(find(trainingData(:,24)~=participant),:); % excluding random participants from data set for validation
...
%     accuracy(i,1) = 1 - kfoldLoss(partitionedModel, 'LossFun', 'ClassifError');
    ...
    confusionmat(response_test,label)
end
