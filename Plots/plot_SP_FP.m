...
for n=1:4
    results_grf.treadmill{n}= cell(1,12);
    for m=1:12
        results_grf.treadmill{n}{m}= NaN(18,5);
    end
end
...
%     A= results_grf.treadmill_SW2{m}(:,1:3);
%     R = corrcoef(A);        % Correlation coefficiant 
%     display(R);
%     try
%         mld=fitlm(results_grf.treadmill_SW2{m}(:,2:3),results_grf.treadmill_SW2{m}(:,1));   % Fit linear regresion model for two variables 
%     catch
%         display(['fitting error for participant ' , num2str(m)])
%     end
...
%     display(i)
%     display(mld)
%             scatter3 (results_grf.treadmill_SW2{m}(1:6,3),results_grf.treadmill_SW2{m}(1:6,1),results_grf.treadmill_SW2{m}(1:6,2),'g')        % 3D scatter plots for 5km/h, forceplate data and smartphone jerk for each participant 
%             hold on
%             scatter3(results_grf.treadmill_SW2{m}(7:12,3),results_grf.treadmill_SW2{m}(7:12,1),results_grf.treadmill_SW2{m}(7:12,2),'r')      % 3D scatter plots for 8km/h, forceplate data and smartphone jerk for each participant
%             hold on
%             scatter3(results_grf.treadmill_SW2{m}(13:18,3),results_grf.treadmill_SW2{m}(13:18,1),results_grf.treadmill_SW2{m}(13:18,2),'b')   % 3D scatter plots for 12km/h, forceplate data and smartphone jerk for each participant
