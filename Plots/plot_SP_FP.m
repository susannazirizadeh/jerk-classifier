...
for n=1:4
    results_grf{n}= cell(1,12);
    for m=1:12
        results_grf{n}{m}= NaN(18,5);
    end
end
        ...
        if isempty( mjerk_grf.treadmill{m}{3}{1}{p} ) ~= 1
            if isempty( mjerk_grf.treadmill{m}{3}{2}{p} ) ~= 1
                if isempty( mjerk_grf.treadmill{m}{3}{3}{p} ) ~= 1         
                    results_grf{n}{m}(:,1)= cell2mat([mjerk_grf.treadmill{m}{5}{1}(1:6)';mjerk_grf.treadmill{m}{5}{2}(1:6)';mjerk_grf.treadmill{m}{5}{3}(1:6)']);
                    results_grf{n}{m}(:,2)= cell2mat([mjerk_grf.treadmill{m}{n}{1}(1:6)';mjerk_grf.treadmill{m}{n}{2}(1:6)';mjerk_grf.treadmill{m}{n}{3}(1:6)']);
                    results_grf{n}{m}(:,3)= ([5,5,5,5,5,5,8,8,8,8,8,8,12,12,12,12,12,12]);
                    results_grf{n}{m}(:,4)=([BWperweight(1:6,m);BWperweight(1:6,m);BWperweight(1:6,m)]);
                    results_grf{n}{m}(:,5)=([m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m]); 
                end
            end
        end
    end
    
%     A= results_grf_SW2{m}(:,1:3);
%     R = corrcoef(A);        % Correlation coefficiant 
%     display(R);
%     try
%         mld=fitlm(results_grf_SW2{m}(:,2:3),results_grf_SW2{m}(:,1));   % Fit linear regresion model for two variables 
%     catch
%         display(['fitting error for participant ' , num2str(m)])
%     end
...
%     display(i)
%     display(mld)
%             scatter3 (results_grf_SW2{m}(1:6,3),results_grf_SW2{m}(1:6,1),results_grf_SW2{m}(1:6,2),'g')        % 3D scatter plots for 5km/h, forceplate data and smartphone jerk for each participant 
%             hold on
%             scatter3(results_grf_SW2{m}(7:12,3),results_grf_SW2{m}(7:12,1),results_grf_SW2{m}(7:12,2),'r')      % 3D scatter plots for 8km/h, forceplate data and smartphone jerk for each participant
%             hold on
%             scatter3(results_grf_SW2{m}(13:18,3),results_grf_SW2{m}(13:18,1),results_grf_SW2{m}(13:18,2),'b')   % 3D scatter plots for 12km/h, forceplate data and smartphone jerk for each participant
