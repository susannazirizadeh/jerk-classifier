...
for m= 1:11
    ...
    display(mld)
            ...
            hold on
            scatter3(results{m}(7:12,3),results{m}(7:12,1),results{m}(7:12,2),'r')
            hold on
            scatter3(results{m}(13:18,3),results{m}(13:18,1),results{m}(13:18,2),'b')
...
for m= 1:11
    ...
    try
        f=fit(results{m}(:,1),results{m}(:,2),'poly1');
    catch
        display(['fitting error for participant ' , num2str(m)])
    end
    i=i+1;
