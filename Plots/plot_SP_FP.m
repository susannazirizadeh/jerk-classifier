...
for m= 1:12
        ...
        if isempty( mjerk_grf.treadmill{m}{3}{1}{p} ) ~= 1
            if isempty( mjerk_grf.treadmill{m}{3}{2}{p} ) ~= 1
                if isempty( mjerk_grf.treadmill{m}{3}{3}{p} ) ~= 1         
                    results_grf{m}(:,1)= cell2mat([mjerk_grf.treadmill{m}{5}{1}(1:6)';mjerk_grf.treadmill{m}{5}{2}(1:6)';mjerk_grf.treadmill{m}{5}{3}(1:6)']);
                    results_grf{m}(:,2)= cell2mat([mjerk_grf.treadmill{m}{4}{1}(1:6)';mjerk_grf.treadmill{m}{4}{2}(1:6)';mjerk_grf.treadmill{m}{4}{3}(1:6)']);
                    results_grf{m}(:,3)= ([5,5,5,5,5,5,8,8,8,8,8,8,12,12,12,12,12,12]);
                    results_grf{m}(:,4)=([BWperweight(1:6,m);BWperweight(1:6,m);BWperweight(1:6,m)]);
                    results_grf{m}(:,5)=([m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m,m]);
                end
            end
        end
    end
    ...
    display(R);
    try
        mld=fitlm(results_grf{m}(:,2:3),results_grf{m}(:,1));   % Fit linear regresion model for two variables 
    catch
        display(['fitting error for participant ' , num2str(m)])
    end
    ...
    display(i)
    display(mld)
            scatter3 (results_grf{m}(1:6,3),results_grf{m}(1:6,1),results_grf{m}(1:6,2),'g')        % 3D scatter plots for 5km/h, forceplate data and smartphone jerk for each participant 
            hold on
            scatter3(results_grf{m}(7:12,3),results_grf{m}(7:12,1),results_grf{m}(7:12,2),'r')      % 3D scatter plots for 8km/h, forceplate data and smartphone jerk for each participant
            hold on
            scatter3(results_grf{m}(13:18,3),results_grf{m}(13:18,1),results_grf{m}(13:18,2),'b')   % 3D scatter plots for 12km/h, forceplate data and smartphone jerk for each participant
