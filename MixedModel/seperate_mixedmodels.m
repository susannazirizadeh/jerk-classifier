...
% Plot
 ...
 for button= 1:4
     ...
     for m=1:12
         x1=results_jerkpos.treadmill{button}{m}(1:6,4); y1=results_jerkpos.treadmill{button}{m}(1:6,2);idx = isfinite(x1) & isfinite(y1);
         try
             f1=fit(x1(idx),y1(idx),'poly1');
         catch
             display(['fitting error for participant ' , num2str(m)])
         end
         x2=results_jerkpos.treadmill{button}{m}(7:12,4) ;y2=results_jerkpos.treadmill{button}{m}(7:12,2); idx = isfinite(x2) & isfinite(y2);
         try
             f2=fit(x2(idx),y2(idx),'poly1');
         catch
             display(['fitting error for participant ' , num2str(m)])
         end
         x3=results_jerkpos.treadmill{button}{m}(13:18,4) ;y3=results_jerkpos.treadmill{button}{m}(13:18,2); idx = isfinite(x3) & isfinite(y3);
         try
...
lme3=  fitlme(tbl,'JerkSmartphone~BW');                       % Mixed model without random effect intercept
 ...
 for button= 1:4
     ...
     for m=1:12
         x1=results_jerkpos.treadmill{button}{m}(1:6,3); y1=results_jerkpos.treadmill{button}{m}(1:6,2);idx = isfinite(x1) & isfinite(y1);
         try
             f1=fit(x1(idx),y1(idx),'poly1');
         catch
             display(['fitting error for participant ' , num2str(m)])
         end
         x2=results_jerkpos.treadmill{button}{m}(7:12,3) ;y2=results_jerkpos.treadmill{button}{m}(7:12,2); idx = isfinite(x2) & isfinite(y2);
         try
             f2=fit(x2(idx),y2(idx),'poly1');
         catch
             display(['fitting error for participant ' , num2str(m)])
         end
         x3=results_jerkpos.treadmill{button}{m}(13:18,3) ;y3=results_jerkpos.treadmill{button}{m}(13:18,2); idx = isfinite(x3) & isfinite(y3);
         try
             f3=fit(x3(idx),y3(idx),'poly1');
         catch
             display(['fitting error for participant ' , num2str(m)])
...
lme3=  fitlme(tbl,'JerkSmartphone~Speed');                       % Mixed model without random effect intercept
 ...
 for button= 1:4
     ...
     for m=1:12
         x1=results_grf.treadmill{button}{m}(1:6,1); y1=results_jerkpos.treadmill{button}{m}(1:6,2);idx = isfinite(x1) & isfinite(y1);
         try
             f1=fit(x1(idx),y1(idx),'poly1');
         catch
             display(['fitting error for participant ' , num2str(m)])
         end
         x2=results_grf.treadmill{button}{m}(7:12,1) ;y2=results_jerkpos.treadmill{button}{m}(7:12,2); idx = isfinite(x2) & isfinite(y2);
         try
             f2=fit(x2(idx),y2(idx),'poly1');
         catch
             display(['fitting error for participant ' , num2str(m)])
         end
         x3=results_grf.treadmill{button}{m}(13:18,1) ;y3=results_jerkpos.treadmill{button}{m}(13:18,2); idx = isfinite(x3) & isfinite(y3);
         try
             f3=fit(x3(idx),y3(idx),'poly1');
         catch
             display(['fitting error for participant ' , num2str(m)])
...
lme3=  fitlme(tbl,'JerkSmartphone~ForceRateFP');                       % Mixed model without random effect intercept
 ...
 %% 4. Plots and model JerkSP-JerkFP
 % Plot
 figure;
 for button= 1:4
     ...
     for m=1:12
         ...
         hold on
         grid on
         semilogx(flip(x1),y1,'r*')
         semilogx(flip(x2),y2,'bo')
         semilogx(flip(x3),y3,'gd')
         %          plot(x1,y1,'r*',x2,y2,'bo',x3,y3,'gd')
