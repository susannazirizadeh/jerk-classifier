...
find(residuals(lme1) < -200);
find(residuals(lme1) >  200);

%%
lme1=  fitlme(tbl,'ForceRateFP~JerkSmartphone+Speed+BW+(1|Participant)+(JerkSmartphone-1|Participant)','Exclude',[61,66,97,102,110]);  % Mixed model with random effect intercept and coefficient 
lme2=  fitlme(tbl,'ForceRateFP~JerkSmartphone^2+Speed+BW+(1|Participant)','Exclude',[61,66,97]);   % Mixed model with random effect intercept 
