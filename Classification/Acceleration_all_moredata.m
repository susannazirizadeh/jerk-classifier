...
%% Transfer into classification_data.acc_all
% Allocat classification_data.acc_all
classification_data.acc_all= cell(1,length(partic));
for m= 1:length(partic)
    classification_data.acc_all{m}= cell(1,length(device));
    for n=1:length(device)
        classification_data.acc_all{m}{n}= cell(1,length(speed));
        for o=1:length(speed)
            classification_data.acc_all{m}{n}{o}= cell(1,length(con));
            for p=1:length(con)
                classification_data.acc_all{m}{n}{o}{p}= [];
            end
        end
    end
end
%% Transform raw_int.outdoor to classification_data.acc_all for SP1 and SW1
                    ...
                    if isempty( raw_int.outdoor{m}{n}{o}) ~= 1
                        for p=1:5%length(con)   % Conditions
                            if isempty( raw_int.outdoor{m}{n}{o}{p}) ~= 1
                                [classification_data.acc_all{m}{n}{o}{p}(:,2),classification_data.acc_all{m}{n}{o}{p}(:,3),classification_data.acc_all{m}{n}{o}{p}(:,4)]=acc_all(raw_int.outdoor{m}{n}{o}{p}(500:end-500,1),raw_int.outdoor{m}{n}{o}{p}(500:end-500,1:4));
                                classification_data.acc_all{m}{n}{o}{p}(:,1)=raw_int.outdoor{m}{n}{o}{p}(500:end-500,1);
                            end
                        end
%                     end
                % end
...
%                     if isempty( raw_int.outdoor{m}{n}{o}) ~= 1
                        for p=6:7 %length(con)   % Conditions
                            if isempty( raw_int.outdoor{m}{n}{o}{p}) ~= 1
                                if isempty( raw_int.outdoor{m}{n}{o}{p}) ~= 1
                                [classification_data.acc_all{m}{n}{o}{p}(:,2),classification_data.acc_all{m}{n}{o}{p}(:,3),classification_data.acc_all{m}{n}{o}{p}(:,4)]=acc_all(raw_int.outdoor{m}{n}{o}{p}(10:end-10,1),raw_int.outdoor{m}{n}{o}{p}(10:end-10,1:4));
                                classification_data.acc_all{m}{n}{o}{p}(:,1)=raw_int.outdoor{m}{n}{o}{p}(10:end-10,1);
                                end
                            end
                        end
                    end
 ...
 % For GPS data
for m= 1:length(partic) %Participants
    if isempty(classification_data.acc_all{m}) ~= 1
        for n=3 
            if isempty(raw_int.outdoor{m}{n}) ~= 1
                for o=1:length(speed)   % Speed
                    if isempty( raw_int.outdoor{m}{n}{o}) ~= 1
                        for p=1:length(con)   % Conditions
                            if isempty(raw_int.outdoor{m}{n}{o}{p}) ~= 1
                                classification_data.acc_all{m}{n}{o}{p}(:,1:3)=raw_int.outdoor{m}{n}{o}{p}(:,1:3);
                            end
                        end
                    end
                end
...
%% Calculating results.acc_all of outdoor session for classifier 
% For smartwatch and smartphone
for m= 1:length(partic)
    if isempty(classification_data.acc_all{m}) ~= 1
        for n=1:2 %length(device)
            if isempty(classification_data.acc_all{m}{n}) ~= 1
                for o=1:length(speed)
                    if isempty(classification_data.acc_all{m}{n}{o}) ~= 1
                        for p=1:5%length(con)
                            if isempty(classification_data.acc_all{m}{n}{o}{p}) ~= 1
                                for i= 1:250:length(classification_data.acc_all{m}{n}{o}{p})
                                    if i+500 < length(classification_data.acc_all{m}{n}{o}{p}) % Windowing, window length= 500 data points (5 seconds) with 50% overlapping
                                        
                                        sum_window = sum(classification_data.acc_all{m}{n}{o}{p}(i:i+499,2));
                                        count_nonzero = sum(classification_data.acc_all{m}{n}{o}{p}(i:i+499,3));
                                ...
                                if isempty(classification_data.acc_all{m}{3}{o}{p}) ~= 1 %GPS with 1Hz sampling frequency , window 5s
                                    for i= 1:3:length(classification_data.acc_all{m}{3}{o}{p})
                                        if i+3 < length(classification_data.acc_all{m}{3}{o}{p})
                                            mloadrate.outdoor{m}{3}{o}{p}((i + 2) / 3,1)= (nanmean(classification_data.acc_all{m}{3}{o}{p}(i:i+2,3)))*3.6;
                        ...
                        for p= 6:7
                            if isempty(classification_data.acc_all{m}{n}{o}{p}) ~= 1
                                for i= 1:50:length(classification_data.acc_all{m}{n}{o}{p})
                                    if i+100 < length(classification_data.acc_all{m}{n}{o}{p}) % Windowing, window length= 500 data points (5 seconds) with 50% overlapping
                                        
                                        sum_window = sum(classification_data.acc_all{m}{n}{o}{p}(i:i+99,2));
                                        count_nonzero = sum(classification_data.acc_all{m}{n}{o}{p}(i:i+99,3));
                ...
                if isempty( mloadrate.outdoor{m}{n}{o} ) ~= 1
                    for  p=1:length(con)
                        if isempty( mloadrate.outdoor{m}{n}{o}{p} ) ~= 1
                            ...
                            results.acc_all{n}{m}(1:length(SP),:)= SP; %loadrate smartphone
                        end
                    end
                end
            end
