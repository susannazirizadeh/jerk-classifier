                    ...
                    if isempty( raw_int.outdoor{m}{n}{o}) ~= 1
                        for p=1:5%length(con)   % Conditions
                            if isempty( raw_int.outdoor{m}{n}{o}{p}) ~= 1
                                ...
                                [classification_data.acc_all{m}{n}{o}{p}(:,2),classification_data.acc_all{m}{n}{o}{p}(:,3),classification_data.acc_all{m}{n}{o}{p}(:,4)]=acc_all(a(500:end-500,1),b(500:end-500,1:4));
                                classification_data.acc_all{m}{n}{o}{p}(:,1)=a(500:end-500,1);
                            end
                        end
                        for p=6:7 %length(con)   % Conditions
                            if isempty( raw_int.outdoor{m}{n}{o}{p}) ~= 1
                                ...
                                [classification_data.acc_all{m}{n}{o}{p}(:,2),classification_data.acc_all{m}{n}{o}{p}(:,3),classification_data.acc_all{m}{n}{o}{p}(:,4)]=acc_all(a(10:end-10,1),b(10:end-10,1:4));
                                classification_data.acc_all{m}{n}{o}{p}(:,1)=a(10:end-10,1);
                                end
                            end
                        end
                    end
            ...
            if isempty(raw_int.outdoor{m}{n}) ~= 1
                for o=1:length(speed)   % Speed
                    if isempty( raw_int.outdoor{m}{n}{o}) ~= 1
                        for p=1:5%length(con)   % Conditions
                            if isempty(raw_int.outdoor{m}{n}{o}{p}) ~= 1
                                classification_data.acc_all{m}{n}{o}{p}(:,1:3)=raw_int.outdoor{m}{n}{o}{p}(500:end-500,1:3);
                            end
                        end
                        for p=6:7%length(con)   % Conditions
                            if isempty(raw_int.outdoor{m}{n}{o}{p}) ~= 1
                                classification_data.acc_all{m}{n}{o}{p}(:,1:3)=raw_int.outdoor{m}{n}{o}{p}(10:end-10,1:3);
                            end
                        end
                    end
                end
                    ...
                    if isempty(classification_data.acc_all{m}{n}{o}) ~= 1
                        for p=1:5%length(con)
                            if isempty(classification_data.acc_all{m}{n}{o}{p}) ~= 1
                                win=500;
                                for i= 1:win/2:length(classification_data.acc_all{m}{n}{o}{p})
                                    if i+win < length(classification_data.acc_all{m}{n}{o}{p}) % Windowing, window length= 500 data points (4 seconds) with 50% overlapping
                                        
                                        sum_window = sum(classification_data.acc_all{m}{n}{o}{p}(i:i+(win-1),2));
                                        count_nonzero = sum(classification_data.acc_all{m}{n}{o}{p}(i:i+(win-1),3));
                                ...
                                win=500;
                                if isempty(classification_data.acc_all{m}{3}{o}{p}) ~= 1 %GPS with 1Hz sampling frequency , window 4s
                                    for i= 1:win/2:length(classification_data.acc_all{m}{3}{o}{p})
                                        if i+win < length(classification_data.acc_all{m}{3}{o}{p})
                                            mloadrate.outdoor{m}{3}{o}{p}((i + (win/2)-1) / (win/2),1)= (nanmean(classification_data.acc_all{m}{3}{o}{p}(i:i+(win-1),3)))*3.6; % 3.6 is to make it from m/s to km/h 
                        ...
                        for p= 6:7
                            if isempty(classification_data.acc_all{m}{n}{o}{p}) ~= 1
                                win=100;
                                for i= 1:win/2:length(classification_data.acc_all{m}{n}{o}{p})
                                    if i+win < length(classification_data.acc_all{m}{n}{o}{p}) % Windowing, window length= 100 data points (5 seconds) with 50% overlapping
                                        sum_window = sum(classification_data.acc_all{m}{n}{o}{p}(i:i+(win-1),2));
                                        count_nonzero = sum(classification_data.acc_all{m}{n}{o}{p}(i:i+(win-1),3));
                                        mloadrate.outdoor{m}{n}{o}{p}((i + (win/2)-1) / (win/2),1)= (sum_window./count_nonzero)*weight(m,1); % mean of 500 data points
                                        mloadrate.outdoor{m}{n}{o}{p}((i + (win/2)-1) / (win/2),2)= o; % speed
                ...
                if isempty( mloadrate.outdoor{m}{n}{o} ) ~= 1
                    for  p=1:length(con)
                        if isempty( mloadrate.outdoor{m}{n}{o}{p} ) ~= 1
                            ...
                            results.acc_all{n}{m}(1:length(SP),1:5)= SP; %loadrate smartphone
                            a=GPS(:,1);
                            results.acc_all{n}{m}(1:length(GPS),6)= a;
                        end
                    end
                end
            end
